...
- 길드
  ...
  - 길드원만 볼 수 있는 컨텐츠 구현
- 프로필
  - 해당 회원이 즐겨찾는 게임 / 가입한 길드 / 작성글 목록 구현
- 메시지
  - 메시지를 대화 목록으로 표시
  - 읽지 않은 메시지 알림 구현
- 정보 수정
...
- 로그인과 회원 가입, 비밀번호 찾기를 **애니메이션**을 이용해 한 화면에서 구현하였다.
- 이메일과 닉네임은 **중복 체크**가 가능하다.
- 휴대폰 번호는 숫자를 입력하면 자동으로 하이픈(-)을 추가해 저장한다.
<br><br>

<img src="https://user-images.githubusercontent.com/46345154/173505719-c39ef461-5d69-4b5b-baad-41acaf792ca2.png" width="50%">&nbsp;&nbsp;◀ 회원가입 화면

#### 비밀번호 찾기
- 비밀번호 찾기는 **이메일 인증**을 통해 진행된다.
...
- 길드에 가입한 멤버가 접속할 경우
  ...
  - 길드마다 하나씩 존재하는 **길드 게시판**은 기능은 일반 게시판과 동일하지만, **길드에 가입한 멤버만** 글을 조회 / 등록 / 수정 / 삭제가 가능하다.
<br><br>
...
<img src="https://user-images.githubusercontent.com/46345154/173501630-0f8200dd-09f5-4e90-8a4a-ce02fa7fb9fd.png" width="50%">&nbsp;&nbsp;◀ 정보 수정

#### 정보 수정
- 자신의 **가입 정보**가 표시된다.
- 변경 버튼을 눌러 항목 각각 개별적으로 정보 변경이 가능하다.
<br><br>

<img src="https://user-images.githubusercontent.com/46345154/173501809-9fcf72e1-7c36-46d0-b62c-7a4360ae56e5.png" width="50%">&nbsp;&nbsp;◀ 관리자 계정의 권한

<img src="https://user-images.githubusercontent.com/46345154/173502416-f26d6c3b-9ef4-455c-a2fb-fa30343cc213.png" width="50%">&nbsp;&nbsp;◀ 관리자가 다른 멤버의 프로필에 접속했을 경우 상단에 차단 버튼 표시

#### 관리자 계정의 권한
- 관리자로 접속하면 헤더에 **관리 페이지**(후술) 링크가 나타난다.
- 관리자는 모든 게시물의 수정 / 삭제 권한을 가지고 있다.
- 길드에 가입하지 않아도 게시물을 보거나 수정 / 삭제 및 길드 멤버 관리가 가능하다.
- 게임 게시판을 새로 추가하거나, 기존 게시판의 정보를 수정하거나 삭제할 수 있다.
- 항상 CRUD 과정 중 세션 체크를 통해 **관리자 여부를 판별**하도록 구현하였다.
- 관리자가 다른 멤버의 프로필에 접속했을 경우 상단에 차단 버튼이 표시된다.
- 권한이 차단된 사용자는 모든 게시물에 대해 등록 / 수정 / 삭제가 불가능해진다.
<br><br>

<img src="https://user-images.githubusercontent.com/46345154/173502639-b5bd8bd6-19d0-4845-aea7-4270b17543a9.png" width="50%">&nbsp;&nbsp;◀ 관리 페이지(게임 추가)

<img src="https://user-images.githubusercontent.com/46345154/173504241-2631511b-8891-4f51-bf3e-135cb14f11bd.png" width="50%">&nbsp;&nbsp;◀ 관리 페이지(차단 목록)

#### 관리 페이지
- 관리자가 헤더의 **관리 페이지** 링크를 통해 접속할 수 있다.
- **게임 추가** 메뉴에서는 게임 정보를 입력해서 새로운 게임 게시판을 추가할 수 있다.
- **차단 목록** 메뉴에서는 차단한 사용자들의 목록을 보고 프로필에 방문하여 정보를 보고, 차단을 해제하거나 메시지를 보낼 수 있다.
