...
<jsp:useBean id="commentDto" class="board.GuildsCommentDto" />
	...
	int game_id = guildDto.getGame_id();
...
<script>
	...
	$(function () {
		...
		$.ajax({
			type : "post",
			url : "guild_bbs_like_process.jsp",
	...
	function loadComments() {
		$.ajax({
			type : "get",
			url : "guild_comments_process.jsp",
	...
	function goDelete() {
		...
		window.location='guild_bbs_deletePro.jsp?num=<%=bbsNum%>&guildId=<%=guildId%>&pageNum=<%=pageNum%>' ;
	}
...
<body>
	...
	<h1><a href="guild_bbs.jsp?id=<%=guildId%>&pageNum=1"><%=guildDto.getGuildName()%></a></h1>
		</div>
		<div class="buttons">
			<input class="title-button" type="button" value="글쓰기" onclick="javascript: location.href='guild_bbs_write.jsp?id=<%=guildId %>&pageNum=<%=request.getParameter("pageNum")%>'">
		</div>
	</div>
		...
		<table id="content">
			...
			<tr>
				<td>
					<div id="content"></div>
					<div id="like-wrap">
						<div id="like-button" onclick="likeFn()"><img src="<%=request.getContextPath() %>/images/thumbsup_black.png" width="20px"></div>
	...
	<div class="buttons">
		<button class="outter-button" onclick="window.location='guild_bbs_update.jsp?id=<%=guildId%>&num=<%=bbsNum%>&pageNum=<%=pageNum%>' ">수정하기</button>
		<button class="outter-button" onclick="goDelete()">글삭제</button>
		<button class="outter-button" onclick="window.location='guild_bbs.jsp?id=<%=guildId%>&pageNum=<%=pageNum%>' ">글목록</button>
	</div>
	</div>
	</div>
