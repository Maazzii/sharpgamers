...
function showAddGame() {
	$("div#init, div#add-game, div#blocked-list").css("display", "none");
	$("div#add-game").css("display", "block");
}
function showBlockedList() {
	$("div#init, div#add-game, div#blocked-list").css("display", "none");
	$("div#blocked-list").css("display", "block");
}
</script>
</head>
<body>
	...
	<div id="wrapper">
		...
		<h1>관리</h1>
			...
			<div class="buttons">
				<input class="title-button" type="button" value="게임 추가" onclick="showAddGame()">
				<input class="title-button" type="button" value="차단 목록" onclick="showBlockedList()">
			</div>
		</div>
	<div id="init">
		<h3>메뉴를 선택해 주세요.</h3>
	...
	<div id="add-game">
		<form name="insert-game" action="insert_game.jsp" method="post">
			...
			<div class="outter-buttons-wrap">
				<div class="buttons">
					...
					<input type="submit" class="outter-button" value="추가">
				</div>
			</div>
		</form>
	</div>
	<div id="blocked-list">
		<div class="content-wrap blocked-wrap">
			<table>
				<%
				List<MemberDto> blockedList = memberDao.getBlockedList();
				int countBlock = 0;
				if (blockedList != null) {
					countBlock = blockedList.size();
				}
				%>
				<tr>
					<th>차단 목록 (<%=countBlock %>)</th>
				</tr>
				<%
				if (blockedList == null) {
				%>
				<tr><td>차단된 멤버가 없습니다.</td></tr>
				<%
				} else {
				...
				<tr>
					<td>
						<div>
							<a href="profile.jsp?id=<%=blockedList.get(i).getMember_idx()%>"><%=blockedList.get(i).getMember_name() %></a>
						</div>
					</td>
				</tr>
				<%
